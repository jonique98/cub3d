...
/*   Updated: 2024/02/16 07:55:58 by sumjo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int *texture_copy(t_data *img)
{
	...
	while (row < img2.height)
	{
		...
		{
			result[row * img2.width + col] = img2.addr_tex[row * img2.width + col];
			col++;
		}
		row++;
	}
	mlx_destroy_image(img2.mlx, img2.img);
	return (result);
}
...
void	draw_map(t_var *var)
{
	...
	while (x < w)
	{
		...
		while (y < var->ray->drawEnd)
		{
			...
			texPos += step;
			unsigned int color = var->texture[0].texture[(int)var->texture[0].width * texY + texX];
			my_mlx_pixel_put(var->image, x, y, color);
			y++;
		}
	x++;
